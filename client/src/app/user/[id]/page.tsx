...
const Page = ({ params: { id } }: Props) => {
	...
	const currentUser = allUserdata[Number(id) - 1]

	return (
		<div className='relative overflow-x-auto shadow-md sm:rounded-lg max-w-[1000px] m-5'>
			<h1 className='text-[30px]'>name : {currentUser?.name}</h1>
				...
				<tbody>
					{currentUser?.currentAlbums.map(({ title, id }) => (
						<tr className='border-b border-gray-200 dark:border-gray-700 cursor-pointer ' onClick={() => router.push(`/album/${id}`)} key={id}>
							<th scope='row' className='px-6 py-4 font-medium text-gray-900 whitespace-nowrap bg-gray-50 hover:bg-slate-500 dark:text-white dark:bg-gray-800'>
								{title}
							</th>
