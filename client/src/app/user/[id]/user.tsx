				...
				<tbody>
					{currentUser?.currentAlbums.map(({ title, id }) => (
						<tr className='border-b border-gray-200 dark:border-gray-700 cursor-pointer ' onClick={() => router.push(`/album/${id}`)} key={id}>
							<th scope='row' className='px-6 py-4 font-medium text-gray-900 whitespace-nowrap bg-gray-50 hover:bg-slate-500 dark:text-white dark:bg-gray-800' data-testid='albumname'>
								{title}
							</th>
							<td className='px-6 py-4' data-testid='albumids'>
								{id}
							</td>
						</tr>
					))}
